class Buttons extends THREE.Mesh {
    constructor() {
        super()
    ...
    genButtons = () => {
        ...
        while (x < 36) {
            ...
            if (multiplier > 8) { multiplier = 0 }
            if (x < 9 || x > 17 && x < 27) {
                button.rotation.y = 0;
                if (x < 9) { button.position.set(84 - multiplier * 21, 1, 112) }
                else { button.position.set(-84 + multiplier * 21, 1, -112) }
            }
            else {
                button.rotation.y = Math.PI / 2; //90deg rotation
                if (x < 18) { button.position.set(-112, 1, 84 - multiplier * 21) }
                else { button.position.set(112, 1, -84 + multiplier * 21) }
            }
